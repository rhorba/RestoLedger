...
## CORRECTION — 2026-08-20 — Sprint 4 (CI caught a real issue: secret-shaped test fixtures)
CI's Gitleaks job failed on commit d0a4816 — flagged `api/src/integrations/credentials-cipher.spec.ts:13`, a test fixture using `sk_live_abc123` and `whsec_xyz` as example credential values. Those aren't real secrets, but `sk_live_`/`whsec_` are Stripe's actual live-key and webhook-secret prefix formats — Gitleaks (correctly) can't tell "obviously fake" from "real" by content alone, and a fixture that's shaped like a real secret is exactly what a scanner should flag regardless of intent. Same pattern was in two other new test files (generic-webhook.provider.spec.ts, integrations.e2e-spec.ts) — fixed all three to use fixture values with no resemblance to a real provider's key format, rather than allowlisting the finding in Gitleaks config. This is exactly what having CI from Sprint 4 onward is for — this went unnoticed through the Sprint 4 batch 2 commit and manual local test runs; only the actual secrets scanner caught it.
