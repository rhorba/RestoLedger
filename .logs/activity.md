...
## MILESTONE — Sprint 4 batch 1 (CI/CD) — 2026-08-20
Story 4.2: GitHub Actions CI pipeline added (.github/workflows/ci.yml) — 4 jobs: secrets-scan (Gitleaks), api (lint+typecheck+test with 80% coverage gate enforced via jest coverageThreshold+Postgres service container), web (lint+build), mobile (lint+typecheck+test). npm audit gated at --audit-level=critical in all three (documented triaged high/moderate dev-tooling issues don't block).
Running actual lint for the first time in this project surfaced real issues: missing @types/bcrypt and @types/passport-jwt (silently degrading type safety to `any` across auth code), an unhandled bootstrap() promise rejection in main.ts, a real Express params typing gap (route params can be string[]), and a Prisma type-inference collapse in TenantMembershipGuard masked by untyped `request: any`. All fixed, not suppressed — logged as a reminder that "typechecks clean" and "lints clean" are different guarantees.
Bumped Jest testTimeout to 15s (from default 5s) after a transient timeout on a loaded local machine — CI runners are typically slower than local dev hardware.

## MILESTONE — Sprint 4 batch 2/3 (backups + compliance checklist) — 2026-08-20
Story 4.3: encrypted backup (pg_dump → gzip → AES-256-CBC/openssl) + restore drill scripts. Actually run against local dev DB, not just written: backup produced (124K), restore drill passed (7 tables, 8 applied migrations, disposable DB auto-cleaned up).
Story 4.4: docs/pre-launch-compliance-checklist.md — separates real legal/CNDP action items (need a human with legal authority) from technical controls already implemented and tested (RLS, encryption at rest, audit trail, backups, dependency/secret scanning). security-restoledger.md STRIDE table and §7 checklist updated to reflect actual current implementation status instead of Sprint-1-era "TODO" placeholders.

## MILESTONE — Sprint 4 SHIP — 2026-08-20
Sprint 4 (Epic 4: POS/payment integration, CI/CD, encrypted backups, pre-launch compliance) complete and pushed to origin/main (9810769). All 4 CI runs for this sprint's final state verified green directly via `gh run list` (not trusted from background-task notifications after one gave a false "success" read on a run that had actually failed — see corrections.md-adjacent lesson, logged here since it's a tooling/process note rather than a code correction: always verify CI status independently, never trust a task-completion notification's exit code as a proxy for the actual result it's reporting on).
Backend: 69 tests passing (was 47 at Sprint 3 end — +22 for POS integration), 98.61%/80.54%/96.38%/98.69% coverage, CI-enforced.
This closes out the PRD's original 4-sprint plan (docs/prd-restoledger.md §8). What's NOT done and cannot be done by an AI agent: actual production deployment (needs the user to choose and pay for a hosting platform — Railway/Render/Fly.io per devops-restoledger.md §3 — none chosen yet), app store publishing (Sprint 3, needs Apple/Google developer accounts), and the legal/CNDP compliance sign-off (Story 4.4, needs a human with legal authority). The codebase, tests, CI pipeline, and documentation are complete and verified through Sprint 4's scope.
